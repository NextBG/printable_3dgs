	...
	dim3 grid)
{
	...
	if (radii[idx] > 0)
	{
		...
		float max_radius = radii[idx];
		float2 p = p_rend_xy[idx];
		rect_min = {
...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char*& chunk, size_t P)
{
	...
	obtain(chunk, geom.rend_means2D, P, 128);
	obtain(chunk, geom.cov3D, P * 6, 128);
	obtain(chunk, geom.opacity, P, 128);
	...
	)
{
	...
	FORWARD::preprocess(
		...
		opacities,
		shs,
		width, height, pix_per_unit,
		slice_height,
		radii,
		geomState.means3D,
		geomState.rend_means2D,
		geomState.depths,
		geomState.cov3D,
		geomState.rgb,
		geomState.opacity,
